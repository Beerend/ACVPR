...
def Bottleneck(med_filters, out_filters, inputs, downsampling=False):
    ...
    b_identity  = BatchNormalization()(Conv1x1(out_filters, (1, 1), 'b_identity', inputs))
    b_conv1     = Conv1x1BatchNormReLU(med_filters, strides, 'b_conv1', inputs)
    b_conv2     = Conv3x3BatchNormReLU(med_filters, (1, 1), 'b_conv2', b_conv1)
    b_conv3     = Conv1x1(out_filters, (1, 1), 'b_conv3', b_conv2)
    b_bn3       = BatchNormalization()(b_conv3)
    b_out       = Activation(activation='relu')(b_bn3 + b_identity)
...
def ResNet101(input_shape):
    ...
    conv2_2     = Bottleneck(64, 256, conv2_1)
    conv2_3     = Bottleneck(64, 256, conv2_2)

    convA_1     = Conv3x3(256, (2, 2), 'convA_1', conv2_3)
    convA_2     = Conv1x1(512, (1, 1), 'convA_2', convA_1)

    conv3_1     = Bottleneck(128, 512, conv2_3, True)
    conv3_2     = Bottleneck(128, 512, conv3_1)
    conv3_3     = Bottleneck(128, 512, conv3_2)
    conv3_4     = Bottleneck(128, 512, conv3_3)

    B_0         = Add()([conv3_4, convA_2])
    convB_1     = Conv3x3(512, (2, 2), 'convB_1', B_0)
    convB_2     = Conv1x1(1024, (1, 1), 'convB_2', convB_1)

    conv4_1     = Bottleneck(256, 1024, conv3_4, True)
    conv4_2     = Bottleneck(256, 1024, conv4_1)
    conv4_3     = Bottleneck(256, 1024, conv4_2)
    conv4_4     = Bottleneck(256, 1024, conv4_3)
    conv4_5     = Bottleneck(256, 1024, conv4_4)
    conv4_6     = Bottleneck(256, 1024, conv4_5)
    conv4_7     = Bottleneck(256, 1024, conv4_6)
    conv4_8     = Bottleneck(256, 1024, conv4_7)
    conv4_9     = Bottleneck(256, 1024, conv4_8)
    conv4_10    = Bottleneck(256, 1024, conv4_9)
    conv4_11    = Bottleneck(256, 1024, conv4_10)
    conv4_12    = Bottleneck(256, 1024, conv4_11)
    conv4_13    = Bottleneck(256, 1024, conv4_12)
    conv4_14    = Bottleneck(256, 1024, conv4_13)
    conv4_15    = Bottleneck(256, 1024, conv4_14)
    conv4_16    = Bottleneck(256, 1024, conv4_15)
    conv4_17    = Bottleneck(256, 1024, conv4_16)
    conv4_18    = Bottleneck(256, 1024, conv4_17)
    conv4_19    = Bottleneck(256, 1024, conv4_18)
    conv4_20    = Bottleneck(256, 1024, conv4_19)
    conv4_21    = Bottleneck(256, 1024, conv4_20)
    conv4_22    = Bottleneck(256, 1024, conv4_21)
    conv4_23    = Bottleneck(256, 1024, conv4_22)

    C_0         = Add()([conv4_23, convB_2])
    convC_1     = Conv3x3(1024, (2, 2), 'convC_1', C_0)
    convC_2     = Conv1x1(2048, (1, 1), 'convC_2', convC_1)

    conv5_1     = Bottleneck(512, 2048, conv4_23, True)
    conv5_2     = Bottleneck(512, 2048, conv5_1)
    conv5_3     = Bottleneck(512, 2048, conv5_2)

    D_0         = Add()([conv5_3, convC_2])
    avgpool     = AveragePooling2D(pool_size=(1, 1), strides=(1, 1), padding='valid')(D_0)
...
def ResNet101HyperParameters(lr):
    ...
    losses = {'out_detect': 'sparse_categorical_crossentropy'}
    '''losses = {
...
def train(model, train_seq, validation_seq, args):
    ...
    model.compile(optimizer = optimizer, loss = losses, loss_weights = loss_weights, metrics = ['accuracy'], callbacks = [reduce_lr])
